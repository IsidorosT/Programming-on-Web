...
int FIND_CROWDED_PLACES(int day, vector<int> time, vector<Node> Square_Region_of_Interest, int Minimum_Stay_Duration, user thisuser) {
	vector<user> in = COVID_CASES;
	in.push_back(thisuser);
	for (int i = day + (time[0] / 30); i <= day; i++) {
...
/*
vector<Node> REPAIR(int day, vector<Node> Trajectory) {
	vector<Node> result;
	for (int i = Trajectory[day].time; i < (time_counter / 30); i++) {
		if (i < day) {
...
int main() {
	...
	day_index.push_back(0);//Last Update
		...
		if (time_counter % 86400 == 0) {
			...
			while (x != "N")
			{
				...
				if (x == "Y") {
					...
					cin >> min;
					k =FIND_CROWDED_PLACES(day_index.size() - 1,time,coordinates,min,user);
