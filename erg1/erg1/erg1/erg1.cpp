...
User Move(User& user) {
	...
	if (user.timelost <= 0) {
		if (temp == 0) {
			...
			user.timelost -= 30;
			if (user.flag) {
				user.GPSSIGNAL = true;	user.timelost = 0;
			}
			user.flag = true;
		}
		else
	...
	if (user.current_x == user.dest_x && user.current_y == user.dest_y) {	//If the user has arrived to his destination
		if (user.GPSSIGNAL) {
			if (user.wait_time > 0)											//If there is still time left to wait there
				...
				if (!user.flag) {
					...
					if (user.current_x - user.radius > 0) {
						temp2 = user.current_x - user.radius;	user.start_x = rand() % temp2 + temp1;
					}
					else {
						temp2 = 0;	user.start_x = rand() % temp1;
					}
					...
					if (user.current_y - user.radius > 0) {
						temp2 = user.current_y - user.radius;	user.start_y = rand() % temp2 + temp1;
					}
					else{
						temp2 = 0;	user.start_y = rand() % temp1;
					}
			...
			user.wait_time = 0;	user.radius = user.radius + user.speed;
		}
	}
	else{
		if (user.GPSSIGNAL){
...
int main() {
	...
	cout << "Enter an integer D. The dimensions of the area will be DxD." << endl; cin >> D;
	//Ask for the number of patients
	cout << "Enter the number of COVID-19 cases:" << endl;	int cases;	cin >> cases;
	...
	while (true) {
		...
		if (time_counter % 86400 == 0) {	//86400 secs are equivalent to 24h
			...
			//user.Trajectory = REPAIR(user.days_index.size(), user, user.Trajectory);
			//Patients
			for (int i = 0; i < COVID_CASES.size(); i++) {
			...
			///		Tests		///
			///		1)		///
			printf("Calling POSSIBLE_COVID_19_INFECTION method for day %i\n", user.days_index.size() - 1);
			POSSIBLE_COVID_19_INFECTION(user, user.days_index.size() - 1, COVID_CASES);

			///		4)		///
			if (user.days_index.size() > 1) {
				/*
				...
				*/
				...
				user.Trajectory = SUMMARIZE_TRAJECTORY(user.days_index.size(), 1, user);
